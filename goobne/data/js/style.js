...
//상품 리스트 넓이 보정
function prdtListWidthBorder() {
	var $prdtList = $(".prdtList");
	$prdtList.each(function () {
		var $tgPrdtList = $(this);
		...
		if ($tgPrdtList.hasClass("prdt3")) {//한줄 3개인 경우
			$itemCol = $(this).find(">ul>li:nth-child(3n+1)");
			$itemCol.addClass("fstCol");

			var n = 3;
			for (i = 0; i < n; i++) {
				$item.eq(i).addClass("fstRow");
			}

		} else {//기본 4개
			$itemCol = $(this).find(">ul>li:nth-child(4n+1)");
			$itemCol.addClass("fstCol");
...
function recipeTab() {
	...
	$fstLi.addClass("fstCol");
	for (i = 0; i < 5; i++) {
		$recipeTabLi.eq(i).addClass("fstRow");
	}
